...
async function selectPaymentMethod(method = PLUS_PAYMENT_METHOD_PAYPAL, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    await performOperationWithDelay({ 
      ...
      kind: 'select', 
      label: `select-payment-method-attempt-${attempt}` 
    }, async () => {
      simulateClick(activeTarget);
    });

    log(`Plus Checkout：第 ${attempt}/${maxAttempts} 次尝试点击并激活 ${config.label} 付款方式...`);

    // 等待选中状态生效，Stripe 状态切换可能需要一点时间，这里每轮最多等待 1.5 秒
    const isNowActive = await waitForPaymentMethodActive(config.id, 1500);
    if (isNowActive) {
      isSwitched = true;
      break;
    }
    ...
    if (attempt < maxAttempts) {
      await sleep(500);
      // 重新获取目标，防止页面发生 DOM 更新或重新渲染导致旧引用失效
      const newTarget = findPaymentMethodTarget(config.id);
      if (newTarget) {
        activeTarget = newTarget;
        console.info(`[MultiPage:plus-checkout] ${config.label} target re-selected for attempt ${attempt + 1}`, summarizeElementForDebug(activeTarget));
      }
    }
  }
